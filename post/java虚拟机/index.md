...
### 双亲委派模型
1. 一个类加载器接收到类加载任务，会先交给父类加载器去完成，只有确认父类加载器无法完成加载任务时，才会尝试进行类加载
2. 类加载器和全限定类名才能确定其在JVM中的唯一性。
3. 打破双亲委派模型
	- 重写 loadClass() 方法 
	- 使用线程上下文类加载器
### 类加载过程
...
4. CMS缺点
	...
	- 空间碎片！
5. Garbage First（G1）
	- 能够建立起`时间停顿模型`的收集器，在一个长度为M毫秒的时间片段内，消耗在垃圾收集器上的时间大概率不超过N毫秒。
	- 也就是说`垃圾回收的时间必须可控`！
	...
	- 解决思路是利用之前用于解决跨代引用的`记忆集`来实现region内存活对象的判断
	- G1中的每个region都有一个对应的`记忆集`，虚拟机发现程序在对Reference类型数据进行写操作时，会产生一个Write Barrier暂时中断写操作，并检查持有Reference的对象和Reference指向的对象是否处于同一个region，如果不是，就把相关信息记录到Reference指向对象所属的region的`记忆集`中
	- 内存分配算法是TLAB
6. G1回收过程
	- 初始标记
	- 并发标记
